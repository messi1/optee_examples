...
AesFileEncryptor::AesFileEncryptor(OpTeeContextManager &ctx) : ctx_(ctx) {}

void AesFileEncryptor::encrypt(const std::string &key_id,
  ...
  FileUtils::writeFile(out_file,
                       std::span(output.data(), op.params[3].value.a));
}

void AesFileEncryptor::decrypt(const std::string &key_id,
                               const std::string &in_file,
                               const std::string &out_file) {
  auto input = FileUtils::readFile(in_file);
